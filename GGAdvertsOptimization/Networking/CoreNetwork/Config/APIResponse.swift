    ...
    init(errorCode: Int?, httpCode: Int?, validation: [APIErrorsModel.Validation]) {
        ...
        if validation.count > 0 {
            ...
            case 403:
                ...
                case 111,112,113:
                    Utils.removeUser()
                    AppDelegate.shared.mainTabbarController?.showMainPopupConfirm(message: "Phiên đăng nhập của bạn đã hết hạn. Mời bạn đăng nhập lại!", okAction: {
                        AppDelegate.shared.makeLogin()
                    })
                    messages.append("Phiên đăng nhập của bạn đã hết hạn. Mời bạn đăng nhập lại!")
                default:
                    break
                }
            case 404:
